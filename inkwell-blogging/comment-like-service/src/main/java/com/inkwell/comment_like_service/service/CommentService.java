...
public class CommentService {
    ...
    private void sendNotifications(Comment comment) {
        try {
            ...
            Map postSummary = restTemplate.getForObject(postBaseUrl + "/api/posts/" + comment.getPostId() + "/summary", Map.class);
            if (postSummary == null) {
                return;
            }
            ...
            if (!authorId.equals(comment.getAuthorId())) {
                ...
                restTemplate.postForEntity(notificationBaseUrl + "/api/notifications", payload, Void.class);
            }
    ...
    private Set<Long> resolveMentionUserIds(String content) {
        ...
        while (matcher.find()) {
            ...
            try {
                Map[] users = restTemplate.getForObject(authBaseUrl + "/api/auth/search?username=" + username, Map[].class);
                if (users != null) {
                    for (Map user : users) {
